...
// * VortexAir True HEPA Filter ($30)
// https://www.homedepot.com/p/LEVOIT-VortexAir-True-HEPA-Replacement-Filter-HEACAFLVNUS0012/317963268
// * Noctua NF-A8 PWM ($20) https://noctua.at/en/nf-a8-pwm
// * 5V to 12V USB Boost Converter ($9)
// https://www.amazon.com/gp/product/B01AY3XLEY/
// * USB-A wall charger (free)
// * This adapter
//
// NOTE: because of the orientation, the fan MUST have hydrodynamic bearings.
// Regular ball bearings will wear down quickly.
...
outer_radius =
    inner_radius + inner_to_foam_width + foam_width + foam_to_outer_width;
height = 147;
...
module filter() {
  union() {
    ...
    color("gray") tube(or = outer_radius - foam_to_outer_width,
                       ir = inner_radius + inner_to_foam_width, h = height + 1,
                       center = false);
  }
}
...
module hood() {
  union() {
    tube(ir = hood_rim_radius, or = hood_radius, h = hood_rim_thickness,
         center = false);
    translate([ 0, 0, -hood_lip_height ])
        tube(ir = hood_rim_radius, or = hood_rim_radius + hood_lip_thickness,
             h = hood_lip_height, center = false);
  }
}
...
module shroud() {
  difference() {
    ...
    cylinder(r1 = hood_radius - shroud_thickness,
             r2 = sqrt(2) * fan_width / 2 - shroud_thickness,
             h = shroud_height);
  }
}
...
translate([ 0, 0, height + fan_mount_height ])
    translate([ 0, 0, -fan_mount_thickness / 2 ]) {
  fan_mount();
