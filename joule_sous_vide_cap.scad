...
//
// This cap fits on top well but does not depress the button very well. I would
// recommend extending the "poker" a little
...
touch_affordance_sphere_radius =
    (touch_affordance_radius ^ 2 + touch_affordance_sphere_depth ^ 2) /
    (2 * touch_affordance_sphere_depth);
...
module cap() {
  difference() {
    ...
    translate([ 0, 0, $preview ? -0.01 : 0 ])
        cylinder(r = cap_outer_radius - cap_thickness,
                 h = cap_increased_thickness_height);
    translate([ 0, 0, cap_increased_thickness_height + ($preview ? -0.02 : 0) ])
        cylinder(r = cap_outer_radius - cap_increased_thickness,
                 h = cap_height - cap_thickness -
                     cap_increased_thickness_height);

    translate([
      touch_affordance_radial_pos, 0,
      cap_height +
          (touch_affordance_sphere_radius - touch_affordance_sphere_depth)
    ...
    translate([ 0, 0, groove_height ]) {
      linear_extrude(groove_width) difference() {
        sector(r = groove_outer_radius, start_angle = groove_sector_start_angle,
               end_angle = groove_sector_end_angle);
        sector(r = cap_inner_radius,
               start_angle = groove_sector_start_angle - 1,
               end_angle = groove_sector_end_angle + 1);
      }
    }
  }
}
...
module retainers() {
  for (x = [ [ 0, 1 ], [ 1, -1 ] ]) {
    translate([
      retainer_radial_pos[x[0]] + x[1] * retainer_thickness / 2, 0,
      retainer_height + retainer_length / 2
    ]) {
      cube([ retainer_thickness, retainer_width, retainer_length ],
           center = true);
      translate([ x[1] * retainer_flap_width / 2, 0 ]) cube(
          [ retainer_flap_width, retainer_flap_thickness, retainer_length ],
          center = true);
    }
  }
}
...
if ($preview) {
  color("white") cylinder(r = cap_outer_radius, h = 200);
  translate([ 0, 0, 200 ]) {
    cap();
    poker();
    retainers();
