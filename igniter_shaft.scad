...
module knob() {
  difference() {
    union() {
      rounded_cylinder(r = knob_diameter / 2, r2 = knob_diameter / 8,
                       h = knob_height);
    ...
    translate([ 0, 0, knob_height - knob_notch_height / 2 ])
        cube([ knob_stopper_diameter, knob_notch_width, knob_notch_height ],
             center = true);
  }
  for (i = [0:flanges - 1]) {
...
module fitted() {
  linear_extrude(fitted_piece_height) { circle(d = fitted_piece_diameter); }
}
...
module gear() {
  linear_extrude(gear_height) {
    ...
    degrees = 360 / teeth;
    for (i = [0:teeth - 1]) {
      rotate([ 0, 0, i * degrees ]) polygon([
        [ 0, 0 ], [ 0, gear_inner_radius ],
...
translate([ 0, 0, gear_height ]) {
  translate([ 0, 0, fitted_piece_height ]) knob();
  fitted();
}
gear();