...
module heatsink_mount() {
  ...
  if ($preview) {
    translate([ -(fan_width + shroud_width + heatsink_width), 20, 20 ])
      ...
      translate([ 0, 0, -fan_width ]) rotate([ 0, 180, 180 ])
          translate([ -199, -147 ]) color("RoyalBlue")
              import("stl/MINI-fan-spacer.stl");
      translate([ 0, 0, -(fan_width + shroud_width + heatsink_width) ])
          translate(-fan_center) color("DarkSlateGray")
              linear_extrude(heatsink_width) quadrant(40, fan_center.x);
    }
  }
  // screw hole + attach to pen arm
  rotate([ 90, 0, 90 ]) linear_extrude(screw_protrusion) difference() {
    union() {
      translate(bottom_screw - [ fan_bracket_width, 0 ]) {
        rotate([ 0, 0, -90 ]) translate([ -fan_bracket_width, 0 ])
      ...
      if (use_upper_screw)
        translate(upper_screw) {
          rotate([ 0, 0, -90 ]) quadrant(screw_radius, screw_radius);
          translate([ -fan_bracket_width, 0 ])
              quadrant(screw_radius + fan_bracket_width, screw_radius);

          translate([ 0, screw_radius + fan_bracket_width ])
              rotate([ 0, 0, 180 ]) square(
                  [ screw_radius, 2 * screw_radius + fan_bracket_width ]);

          translate([ -screw_radius, screw_radius ]) rotate([ 0, 0, 180 ])
              right_triangle(holder_z - 2 * screw_radius, 2 * screw_radius);
        }
    }
    ...
    if (use_upper_screw)
      translate(upper_screw) circle(d = screw_hole_diameter);
  }

  union() {
    // use heatsink for alignment
    translate([ -heatsink_width, 0, 0 ]) rotate([ 90, 0, 90 ])
    ...
    translate([ -heatsink_width, 0, 0 ]) rotate([ 90, 0, 90 ])
      ...
      translate([ screw_radius, 0 ]) rotate([ 0, 0, 180 ]) difference() {
        quadrant(screw_radius, screw_radius);
        square([ screw_radius / 2, screw_radius ]);
      }
    }
    if (use_upper_screw) {
      rotate([ 90, 0, 90 ]) linear_extrude(screw_protrusion)
          translate([ -fan_bracket_width, screw_radius ] + upper_screw)
              translate([ 0, screw_radius / 2 ]) rotate([ 0, 180, 90 ]) {
        square([ fan_bracket_width, fan_center.y - screw_radius ]);
        translate([ screw_radius, 0 ]) rotate([ 0, 0, 180 ]) difference() {
          quadrant(screw_radius, screw_radius);
          square([ screw_radius / 2, screw_radius ]);
    ...
    difference() {
      ...
      translate([ 0, -max_pen_size / 2, height / 2 ]) rotate([ 90, 0, 0 ])
          male_metric_thread(thumbscrew_diameter,
                             ...
                             thread_length, top = -1, bot = -1, center = false);
    }
  }
}
